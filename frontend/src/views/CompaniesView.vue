...
import type { Company, CompanyForm, PersonType } from '@/types'
...
const formTitle = computed(() => (editingId.value ? 'Editar empresa' : 'Nueva empresa'))
...
async function loadCompanies(page = 1) {
  ...
  error.value = ''
  try {
    const res = await companiesApi.fetchCompanies(page)
    list.value = res.data
    meta.value = res.meta
  } catch (e: unknown) {
...
async function submitForm() {
  ...
  try {
    ...
    if (editingId.value) {
      await companiesApi.updateCompany(editingId.value, payload)
    } else {
      await companiesApi.createCompany(payload as CompanyForm)
    }
    closeForm()
...
<template>
  <div class="companies-page">
    ...
    <template v-else>
      ...
      <p v-if="list.length === 0" class="empty">No hay empresas. Crea una con «Nueva empresa».</p>
      ...
      <div v-if="meta.last_page > 1" class="pagination">
        <button
          ...
          @click="loadCompanies(meta.current_page - 1)"
        >
          Anterior
        </button>
